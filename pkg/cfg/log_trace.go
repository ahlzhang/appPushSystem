...
func LogInfoSoapf(format string, v ...interface{}) {
	if _log_level > LOG_INFO {
		return
	}
	str := formatLogf(LOG_INFO, default_calldepth, format, v...)
	if logger := GetLogger("soap"); logger != nil {
		logger.Println(str)
	}
}


func LogInfoGrpcf(format string,v ...interface{}) {
	if _log_level > LOG_ERROR {
		return
	}
	str := formatLogf(LOG_ERROR, default_calldepth, format, v...)
	if logger := GetLogger("service"); logger != nil {
		logger.Println(str)
	}
}
...
// 测试性能，打印耗时
func TimeCostLog(tag string, start time.Time, timeLimit float64) int64 {
	dis := time.Now().Sub(start).Seconds()
	d := time.Now().Sub(start)
	if dis > timeLimit {
		LogDebugf("%v:%v, StartTime:%v ", tag, d, start.Format("2006-01-02 15:04:05"))
	}
	// 返回毫秒
